...
void CUICompound::Render()
{
	...
	// Text in 
	_pUIMgr->GetDrawPort()->PutTextEx( _S( 723, "힘의 상자" ), m_nPosX + COMPOUND_TITLE_TEXT_OFFSETX,			
				...
				case CItemData::ITEM_ETC_REFINE:
					{
						...
						if( ulItemFlag > 0 )
						{
							strTemp.PrintF( _S( 160, "레벨: %d" ), ulItemFlag );
...
void CUICompound::SetCompoundItem ( int nSlotIndex )
{
	...
	{
		_pUIMgr->GetChatting()->AddSysMessage( _S( 728, "착용한 아이템은 합성할 수 없습니다." ), SYSMSG_ERROR );	
		_pUIMgr->ResetHoldBtn();
		return;
	}
...
//	{	
	...
	*/		
		...
		if( rItemData.GetType() == CItemData::ITEM_ETC && 
			...
			rItemData.GetRefineType() == CItemData::REFINE_GENERAL )  // 일반제련석
		{
			nSlotIndex = ITEM_ARCANE_MATERIAL_UPGRADE;
		}
		else if( rItemData.GetType() == CItemData::ITEM_ETC && 
				rItemData.GetSubType() == CItemData::ITEM_ETC_SAMPLE ) // 시료
		{
			nSlotIndex = ITEM_ARCANE_MATERIAL_SAMPLE;
		}
		else if( rItemData.GetType() == CItemData::ITEM_ACCESSORY ) // 악세서리
		{
			nSlotIndex = ITEM_ARCANE_MATERIAL_ACCESSORY;
		}
		else
		{
			_pUIMgr->GetChatting()->AddSysMessage( _S( 729, "합성 할 수 있는 아이템이 아닙니다." ), SYSMSG_ERROR ); 
			_pUIMgr->ResetHoldBtn();
			return;
		}
...
void CUICompound::SendCompoundReq()
{
	...
	{
		_pUIMgr->GetChatting()->AddSysMessage( _S( 868, "스킬 사용중에는 힘의 상자를 합성할 수 없습니다." ), SYSMSG_ERROR );	
		return;
	}

	if( _pUIMgr->IsCSFlagOn( CSF_TELEPORT ) )
	{
		_pUIMgr->GetChatting()->AddSysMessage( _S( 869, "순간 이동중에는 힘의 상자를 합성할 수 없습니다." ), SYSMSG_ERROR );	
		return;
	}
