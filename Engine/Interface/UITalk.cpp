...
void CUITalk::Open( CTString strName)
{
	...
	{
		AddTalkListString( "", _S( 1641, "대화상대가 [자리비움] 상태이므로 응답하지 않을 수도 있습니다."), true, TALK_SYSTEM_COLOR ); 
	}
	else
	{
...
void CUITalk::SendChatMessage( CTString strMessage )
{
	...
	{
		AddErrorTalk( _S( 1646, "[오프라인] 상태에서는 대화를 할 수 없습니다." ) ); 
		return;
	}
	...
	for(int i=0; i<m_vecTarget.size(); ++i)
	{
		...
		{
			//AddErrorTalk( _S( 1645, "[오프라인] 상태인 대상과는 대화를 할 수 없습니다." ) ); 
			continue;
	}
		else
