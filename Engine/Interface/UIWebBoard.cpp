...
BOOL CCommandModify::ExecuteDelay(CUIWebBoard &rUIWebBoard, BOOL &bRender)
{
	...
	if(rUIWebBoard.m_web.Read(strContent, strError))
	{
		if(strError.size() == 0)
		{
			...
			else ASSERTALWAYS("리턴정보관련해서 문제있음. 웹팀과 의논할 것");
			return TRUE;
		}
...
void CUIWebBoard::RenderListCommon()
{
	...
	_pUIMgr->GetDrawPort()->PutTextEx( _S( 341, "제목" ), m_nPosX + WBOARD_LIST_TITLE_SUBJECT_SX,
										m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	if( m_nCurSection == UWS_HELP || m_nCurSection == UWS_NOTICE )
	{
		_pUIMgr->GetDrawPort()->PutTextEx( _S( 244, "작성자" ), m_nPosX + WBOARD_LIST_TITLE_NOTICE_NAME_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		_pUIMgr->GetDrawPort()->PutTextEx( _S( 398, "작성일" ), m_nPosX + WBOARD_LIST_TITLE_NOTICE_DATE_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	}
	else if( m_nCurSection == UWS_FREEBOARD || m_nCurSection == UWS_GUILD )
	{
		_pUIMgr->GetDrawPort()->PutTextEx( _S( 244, "작성자" ), m_nPosX + WBOARD_LIST_TITLE_NAME_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		_pUIMgr->GetDrawPort()->PutTextEx( _S( 398, "작성일" ), m_nPosX + WBOARD_LIST_TITLE_DATE_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
		_pUIMgr->GetDrawPort()->PutTextEx( _S( 399, "조회" ), m_nPosX + WBOARD_LIST_TITLE_HIT_SX,
											m_nPosY + WBOARD_LIST_TITLE_SY, 0xFFFFCAFF );
	}
}
...
void CUIWebBoard::RenderWriteCommon()
{
	...
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 244, "작성자" ), nX,
										m_nPosY + WBOARD_WRITE_TITLE_NAME_SY, 0xFFFFCAFF );
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 195, "제  목" ), nX,
										m_nPosY + WBOARD_WRITE_TITLE_SUBJECT_SY, 0xFFFFCAFF );
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 418, "내  용" ), nX,
...
//안태훈 수정 끝	//(5th Closed beta)(0.2)
	...
	// Text in web board
	// Title
	_pUIMgr->GetDrawPort()->PutTextEx( _S( 385, "게시판" ), m_nPosX + WBOARD_TITLE_OFFSETX,
	...
	nX += WBOARD_TAB_WIDTH;
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 252, "공지사항" ), nX, nY,
											m_nCurSection == UWS_NOTICE ? 0xE1B300FF : 0x6B6B6BFF );
	nX += WBOARD_TAB_WIDTH;
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 262, "게시판" ), nX, nY,
											m_nCurSection == UWS_FREEBOARD ? 0xE1B300FF : 0x6B6B6BFF );
	nX += WBOARD_TAB_WIDTH;
	_pUIMgr->GetDrawPort()->PutTextExCX( _S( 865, "길드" ), nX, nY,	
			...
			if( IsInside( nX, nY ) )
			{
				...
				else if( IsInsideRect( nX, nY, m_rcTab ) )
				{
					...
					if( nCurTab >= 0 && nCurTab < UWS_TOTAL )
					{
						...
						{
							_pUIMgr->GetChatting()->AddSysMessage( _S(3883, "길드 관리내 길드 게시판을 이용해 주십시오." ) );
						}
						else 
#endif
