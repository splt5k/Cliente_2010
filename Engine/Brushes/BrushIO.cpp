...
#define BPOV_POLYGONATTRIBUTE			5
#define BPOV_CURRENT                    BPOV_POLYGONATTRIBUTE
...
void CBrushMip::Read_new_t( CTStream *pistrm) // throw char *
{
	...
	if (!bWithMipDistance) {
		...
		if (bm_fMaxDistance==100.0f) {
			...
			bm_fMaxDistance = fPerspectiveRatio*pow(2, bm_fMaxDistance)/1024; //by kevin
		}
	}
...
void CBrushSector::Write_t( CTStream *postrm) // throw char *
{
	...
	{FOREACHINSTATICARRAY(bsc_abpoPolygons, CBrushPolygon, itbpo) {
		...
		// write the attribute of a polygon			// yjpark
		(*postrm) << bpo.bpo_ubPolygonAttribute;	// yjpark
	}}
...
void CBrushSector::Read_t( CTStream *pistrm) // throw char *
{
	...
	{FOREACHINSTATICARRAY(bsc_abpoPolygons, CBrushPolygon, itbpo) {
		...
		{FOREACHINSTATICARRAY(bpo.bpo_abpePolygonEdges, CBrushPolygonEdge, itbpe) {
			...
			//월드 파일을 로딩할 때 튕기는 문제때문에 추가...
			//뭔가 잠재적인 문제가 있지 않을까???...			:Su-won
			if( iEdge >ctEdges-1 )
				iEdge =ctEdges-1;
...
//안태훈 수정 끝	//(Modify Worldbase Overbright to NonOver)(0.1)
		...
		if( iBPOVersion >= BPOV_POLYGONATTRIBUTE )
		{
			(*pistrm) >> bpo.bpo_ubPolygonAttribute;
		}												// yjpark     -->|
	}}
