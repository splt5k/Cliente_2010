...
public:
	...
	__forceinline Quaternion<Type> operator+(const Quaternion<Type>& q2) const;
	__forceinline Quaternion<Type>& operator+=(const Quaternion<Type>& q2);
...
//안태훈 수정 시작	//(Remake Effect)(0.1)
	...
	friend __forceinline CTStream& operator<<(CTStream& strm, const Quaternion<Type>& quat) {
...
// addition/substraction
template<class Type>
__forceinline  Quaternion<Type> Quaternion<Type>::operator+(const Quaternion<Type>& q2) const {
	return Quaternion<Type>(q_w + q2.q_w, q_x + q2.q_x, q_y + q2.q_y, q_z + q2.q_z);
}
template<class Type>
__forceinline  Quaternion<Type>& Quaternion<Type>::operator+=(const Quaternion<Type>& q2) {
...
// spherical linear interpolation
template<class Type>
__forceinline Quaternion<Type> Slerp(const Type tT, const Quaternion<Type>& q1, const Quaternion<Type>& q2)
{
	Type tCos = q1 % q2;
	// ASSERT( tCos>=-1.0015 && tCos<=+1.0015);
	...
	Type tF1, tF2;
	if (tCos < Type(0.99)) {  // ! 0.999 for more precision?
		// standard case (slerp)
		Type tAngle = acos(tCos);
		Type t1oSin = 1 / sin(tAngle);
		tF1 = sin((Type(1) - tT) * tAngle) * t1oSin;
		tF2 = sin(tT * tAngle) * t1oSin;
	...
	const Quaternion<Type>& qa, const Quaternion<Type>& qb)
{
	...
	Quaternion<Type> qS1 = Slerp(tT, q1, q2);
	Quaternion<Type> qS2 = Slerp(tT, qa, qb);
	return Slerp(2 * tT * (1 - tT), qS1, qS2);
}
