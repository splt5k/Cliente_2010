...
*/
		...
		//return CMovableModelEntity::AdjustShadingParameters(vLightDirection, colLight, colAmbient); // 그냥 TRUE만 리턴
		return TRUE;
	};
 ...
 ************************************************************/
	...
	void SpawnBlood_client(EEnemyBaseDamage eEBDamage)		// 안쓰이는 부분.
	{
...
//0322 kwon
	...
	void AttackNow()
	{
		// NOTE : 싱글 모드일 경우에는 AttackTarget를 애니메이션 중에 호출하기때문에,
		// NOTE : 서버에서 메세지를 받고서 공격을 하지는 않는다.
		if( _pNetwork->m_bSingleMode )
		{
			return;
		}
...
//강동민 수정 시작 싱글 던젼 작업	07.23
	SingleActive(EVoid)
	{
		...
		wait () 
		{
			...
			on (EReconsiderBehavior) : 
			{
				if( m_penEnemy != NULL && m_penEnemy->IsFlagOn(ENF_ALIVE) ) //공격 또는 공격시 rotate
				{	
					//SetFlags(GetFlags()|ENF_ALIVE);
					//m_penEnemy이 있다면 attack을 해야한다.
			...
			// if enemy has been seen(플레이어가 보인다면?)
			on (EWatch eWatch) : 
			{
				// if new enemy
	...
	Inactive(EVoid) 
	{
		...
		wait() 
		{
			...
			{ 
				if(m_bNpc)
				{
					call IdleLoop(); //0125 이곳이 NPC들 제스처 호출하는 부분
					//jump Active(); 
				}
				else 
				{
...
//		ASSERT(m_fMaxHealth==GetHealth() || IsOfClass(this, "Devil") || IsOfClass(this, "ExotechLarva") || IsOfClass(this, "AirElemental") || IsOfClass(this, "Summoner"));
		...
		// if this is a template(EnemySpawn을 위한 부분)
		if (m_bTemplate) 
		{
			// do nothing at all
			return;
...
//		ASSERT(m_fAttackDistance>m_fCloseDistance);
	...
	StandardBehavior(EVoid)
	{
		...
		wait()	// 무한 루프
		{
			...
			// if dead
			// 에너미가 죽었다면...(EEnemyBaseDeath를 전달함...)
			on (EDeath eDeath) : 
			{
